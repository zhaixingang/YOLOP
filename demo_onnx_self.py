...
model_path = './onnx/yolop_self.onnx'
yolop_ort_session = ort.InferenceSession(model_path)
yolop_input_name = yolop_ort_session.get_inputs()[0].name

class yolop():
    def __init__(self, confThreshold=0.25, nmsThreshold=0.5, objThreshold=0.45):
        self.classes = "car"   ###这个是在bdd100k数据集上训练的模型做opencv部署的，如果你在自己的数据集上训练出的模型做opencv部署，那么需要修改self.classes
    ...
    def detect(self, srcimg):
        ...
        outs = yolop_ort_session.run(None, {yolop_input_name: blob.astype(np.float32)})
        # inference output
        outimg = srcimg.copy()
        drive_area_mask = outs[3][0][:, padh:(self.inpHeight - padh), padw:(self.inpWidth - padw)]
        ...
        lane_line_mask = outs[4][0][:, padh:(self.inpHeight - padh), padw:(self.inpWidth - padw)]
        ...
        for i in range(self.nl):
            det_out = outs[i][0]
            det_out = np.reshape(det_out, [-1, 6])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--imgpath", type=str, default='inference/images/0ace96c3-48481887.jpg', help="image path")
